...
const product = await stripe.products.create({
  ...
  description: 'Monthly saved quote history, follow-up sequences, industry script drops, and quote performance tracking for home-service businesses.',
  metadata: { project: 'quotesprint', tier: 'live' },
});
...
data.projects.quotesprint.stripe.subscriptionPriceIds.live = price.id;
...
for (const target of ['production', 'preview', 'development']) {
  const args = ['env', 'add', 'STRIPE_PRICE_ID_LIVE_MONTHLY', target, '--token', vercelToken];
  try {
    run('/data/.npm-global/bin/vercel', args, price.id + '\n');
  } catch (error) {
    const msg = String(error.message || error).toLowerCase();
    if (msg.includes('already exists') || msg.includes('already been added')) {
      spawnSync('/data/.npm-global/bin/vercel', ['env', 'rm', 'STRIPE_PRICE_ID_LIVE_MONTHLY', target, '--yes', '--token', vercelToken], { cwd: root, encoding: 'utf8' });
      run('/data/.npm-global/bin/vercel', args, price.id + '\n');
    } else {
      throw error;
    }
...
console.log('LIVE_MONTHLY_PRICE_ID', price.id);
console.log('VERCEL_ENV_SET STRIPE_PRICE_ID_LIVE_MONTHLY');
