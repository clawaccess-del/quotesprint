...
export function QuoteBuilder({ accountEmail, aiEnabled }: { accountEmail?: string; aiEnabled?: boolean }) {
  ...
  const [idealCustomer, setIdealCustomer] = useState('homeowners who want clear communication and dependable service');
  ...
  useEffect(() => {
    ...
    if (profileRaw) {
      try {
        ...
        setCompanyOffer(profile.companyOffer || '');
        setIdealCustomer(profile.idealCustomer || 'homeowners who want clear communication and dependable service');
    ...
    fetch('/api/account', { cache: 'no-store' })
      ...
      .then((data) => {
        ...
        if (data.profile) {
          ...
          setIdealCustomer(data.profile.idealCustomer || 'homeowners who want clear communication and dependable service');
  ...
  }, [business, businessIndustry, companyLogoUrl, companyPhone, companyWebsite, companyOffer, idealCustomer, serviceArea, brandVoice, differentiator, guarantee, accountLoaded]);

  useEffect(() => {
    setCustomizedCopy({});
    setAiStatus('Generated text updated from the current quote options.');
  }, [business, businessIndustry, companyLogoUrl, companyPhone, companyWebsite, companyOffer, idealCustomer, serviceArea, brandVoice, differentiator, guarantee, customer, jobType, laborHours, laborRate, materials, urgency, deposit, tone]);
  ...
  const stageTemplates = useMemo(() => {
    ...
    return [
      ...
      { stage: 'Estimate sent', text: `Hi ${firstName}, I sent over the ${jobType.toLowerCase()} estimate. The working total is ${money(result.total)}, and ${money(result.depositDue)} reserves the appointment. Customers choose ${business} for ${businessIndustry} work because ${differentiator}. Best fit: ${idealCustomer}. Do you want me to hold the next available opening?` },
    ...
    <>
      ...
      {activeTab === 'company' ? <section className="portal-panel-grid single company-info-grid">
        <article className="builder-panel company-info-panel">
          ...
          <div className="two-col"><label>Service area<input value={serviceArea} onChange={(e) => setServiceArea(e.target.value)} placeholder="Charleston, SC" /></label><label>Phone<input value={companyPhone} onChange={(e) => setCompanyPhone(e.target.value)} placeholder="(555) 555-5555" /></label></div>
          <label>Website<input value={companyWebsite} onChange={(e) => setCompanyWebsite(e.target.value)} placeholder="https://your-site.com" /></label>
          ...
          <label>Ideal customer<textarea value={idealCustomer} onChange={(e) => setIdealCustomer(e.target.value)} rows={3} placeholder="Busy homeowners, property managers, commercial clients..." /></label>
          <label>Signature offer or specialty<textarea value={companyOffer} onChange={(e) => setCompanyOffer(e.target.value)} rows={3} placeholder="Same-day repairs, premium repaints, maintenance plans..." /></label>
