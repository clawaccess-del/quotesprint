...
export function QuoteBuilder({ accountEmail, aiEnabled }: { accountEmail?: string; aiEnabled?: boolean }) {
  ...
  const result = useMemo(() => {
    ...
    const customerSpecifics = customerContext.summary;
  ...
  function companyAiContext(extra = '') {
    ...
    return [
      ...
      `Customer-specific context: ${customerContext.summary}`,
      `Customer notes/requested service: ${customerContext.notes || 'none'}`,
      `Customer next step: ${customerContext.nextStep || 'not set'}`,
      `Customer address/location: ${customerContext.address || 'not set'}`,
      `Customer appointment/date: ${customerContext.appointmentDate || 'not set'}`,
      `Customer lead source: ${customerContext.source || 'not set'}`,
  ...
  async function enhanceCopy(title: string, text: string, action = 'rewrite') {
    ...
    const company = companyAiContext(`AI enhancement target: ${title}\nCustomer-specific context to prioritize: ${customerContext.summary}`);
      ...
      {activeTab === 'leads' ? <section className="portal-panel-grid single lead-management-layout">
        ...
        <article className="copy-card lead-workflow-card">
          ...
          {leadWorkflow.currentLead ? <>
            ...
            <div className="hero-actions"><button type="button" className="button" onClick={() => startQuoteFromLead(leadWorkflow.currentLead!)}>Generate quote</button><button type="button" className="button secondary" onClick={() => moveCurrentLead('qualified')}>Qualified</button><button type="button" className="button secondary" onClick={saveQuote}>Save quote + mark quoted</button><button type="button" className="button secondary" onClick={() => moveCurrentLead('followed-up')}>Followed up</button><button type="button" className="button" onClick={() => moveCurrentLead('won')}>Won</button><button type="button" className="button secondary" onClick={() => moveCurrentLead('lost')}>Lost</button></div>
